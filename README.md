...
`force_custom_adventure` | false | NYI force new games to only allow the specified adventure from `custom_adventure_code`. This **OVERRIDES** launch_resume & only allows new games
`custom_adventure_code` | ftk | NYI the config code for the forced adventure. Only used if `force_custom_adventure` is true
...
hildebrants cellar is a dungeon runner only
other adventures use hex movement


## Current State  

v0.8.6: Tony was let loose for a long time with no game breaking issues. praise be the rng gods  
